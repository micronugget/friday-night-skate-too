...
| Git (write, local) | `git add`, `git commit`, `git checkout`, `git checkout -b`, `git stash`, `git merge`, `git rebase`, `git cherry-pick`, `git push origin <feature-branch>` (non-force, feature/issue branches only), `git push --force-with-lease origin <feature-branch>` (feature/issue branches only — safe after rebase; lease check prevents overwriting unexpected remote changes) |
