...
/**
 * Source plugin for legacy fridaynightskate.com `/news` pages.
 *
 * Crawls the paginated news index, then visits each detail page and yields a
 * row with the canonical fields used by the `news` content type migration:
 * `{id, slug, url, title, published_date, hero_image_url, body_html}`.
 *
 * HTTP traffic is delegated to the shared {@see FnsHttpClient} so every
 * response is cached on disk under `private://fns_migrate_cache/news/` and
 ...
 *   index_path:  Path of the paginated index. Default '/news'.
 ...
 * )
 */
class FnsNewsHtml extends SourcePluginBase implements ContainerFactoryPluginInterface {

  protected const DEFAULT_BASE_URL = 'https://fridaynightskate.com';
  protected const DEFAULT_INDEX_PATH = '/news';
  ...
  /**
   * Constructs a FnsNewsHtml source plugin.
   *
  ...
  public function __toString(): string {
    return 'fns_news_html:' . $this->baseUrl . $this->indexPath;
  }
  ...
  public function fields(): array {
    return [
      ...
      'slug' => $this->t('URL slug for the news article.'),
      ...
      'body_html' => $this->t('Body HTML of the article.'),
    ];
  }
  ...
  protected function initializeIterator(): \Iterator {
    ...
    while ($walked < $this->maxPages) {
      ...
      $cacheKey = 'index-' . $page;
      $html = $this->httpClient->fetch($indexUrl, 'news', $cacheKey);
      $links = $this->extractNewsLinks($html);
      if ($links === []) {
        break;
      }
      ...
      foreach ($links as $detailUrl) {
        ...
        $detailHtml = $this->httpClient->fetch($detailUrl, 'news', $slug);
  ...
  /**
   * Extract absolute URLs of news detail pages from an index page.
   *
   * @return string[]
   *   Absolute URLs in document order, deduplicated.
   */
  protected function extractNewsLinks(string $html): array {
  ...
  /**
   * Return TRUE when the URL points at a news detail page.
   */
  ...
  /**
   * Parse a news detail page into a row.
   *
  ...
  protected function parseDetail(string $url, string $slug, string $html): ?array {
    ...
    $title = $this->firstText($crawler, ['h1.news-title', 'article h1', 'h1']);
    if ($title === '') {
      return NULL;
    }
    ...
    return [
      ...
      'url' => $url,
      'title' => $title,
      'published_date' => $this->extractPublishedDate($crawler),
